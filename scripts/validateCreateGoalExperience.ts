...
requireCondition(due.includes('<FlatList') && due.includes('horizontal') && due.includes('pagingEnabled'), 'Due picker does not use a native horizontal FlatList.');
requireCondition(due.includes('data={monthsData}') && due.includes('getItemLayout={') && due.includes('initialScrollIndex={activeIndex}'), 'Due picker FlatList missing stable data, layout, or initial index.');
requireCondition(due.includes('removeClippedSubviews={false}'), 'Due picker FlatList must prevent visual popping on iOS.');
requireCondition(due.includes('Math.round(x / calendarWidth)'), 'Due picker active month is not derived from native scroll offset.');
requireCondition(due.includes('flatListRef.current?.scrollToIndex'), 'Due picker arrows do not use native programmatic scrolling.');
requireCondition(!due.includes('calendarPanResponder') && !due.includes('monthProgress') && !due.includes('incomingMonth') && !due.includes('monthAnimatingRef') && !due.includes('swipeActiveDirectionRef') && !due.includes('lastProcessedDirectionRef') && !due.includes('monthDirection'), 'Obsolete custom PanResponder pager state returned.');
